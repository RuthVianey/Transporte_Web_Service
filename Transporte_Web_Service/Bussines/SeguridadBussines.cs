...
namespace Transporte_Web_Service.Bussines
{
    public class SeguridadBussines
    {
        ...
        public async Task<ApiResponse<IEnumerable<Entity_RespuestaGeneral?>>> Bs_Sucursal_Desactivar(int IdSucursal, int IdEmpresa)
        {
            if (IdEmpresa <= 0)
            {
        ...
        public async Task<ApiResponse<IEnumerable<Entity_RespuestaGeneral?>>> Bs_Sucursal_Guardar(int IdSucursal, int IdEmpresa, string Nombre, string NombreCorto, string Codigo, string Calle, string Colonia, string Municipio, string Estado, string CodigoPostal, string Telefono, byte Activo)
        {
            if (IdEmpresa <= 0)
            {
                return ApiResponse<IEnumerable<Entity_RespuestaGeneral?>>.Fail("La empresa es obligatoria.");
        ...
        public async Task<ApiResponse<IEnumerable<Entity_Sucursal_Listar?>>> Bs_Sucursal_Listar(int IdEmpresa, byte SoloActivos, string TextoBusqueda)
        {
            ...
            if (resumen == null)
            {
                return ApiResponse<IEnumerable<Entity_Sucursal_Listar?>>.Fail("No se encontró información.");
            }
            return ApiResponse<IEnumerable<Entity_Sucursal_Listar?>>.Success(resumen);
        }

        public async Task<ApiResponse<IEnumerable<Entity_Sucursal_Listar?>>> Bs_Sucursal_ObtenerPorId(int IdSucursal, int IdEmpresa)
        {
            if (IdEmpresa <= 0)
            {
                return ApiResponse<IEnumerable<Entity_Sucursal_Listar?>>.Fail("La empresa es obligatoria.");
